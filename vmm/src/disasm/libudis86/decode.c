...
search:
    ...
    case UD_Id3vil:
      // invalid instruction mnemonic constant Id3vil
      u->error = 1;
      return -1;

    default:
      // invalid instruction mnemonic constant
      u->error = 1;
      return -1;
    }
 ...
 */
static enum ud_type 
resolve_reg(struct ud __attribute__((unused)) *u, unsigned int type, unsigned char i)
{
  switch (type) {
...
static int gen_hex( struct ud *u )
{
  ...
  /* sz      = sizeof( u->insn_hexcode ); */
  /* for each byte used to decode instruction */
  /* for ( i = 0; i < u->inp_ctr; ++i, ++src_ptr) { */
  /*    snprintf( src_hex, sz, "%02x", *src_ptr & 0xFF ); */
  /*    src_hex += 2; */
  /*    sz -= 2; */
  /* } */
  return 0;
